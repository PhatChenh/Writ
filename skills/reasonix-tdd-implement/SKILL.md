---
name: reasonix-tdd-implement
version: 1.0.0
description: Use when executing implementation phases from a plan file using TDD (Reasonix port). Triggers on "implement phase", "build next phase", "implement feature", "tdd implement", or when user wants to start coding against an existing plan.
---

# TDD Implementation (Reasonix)

> **Reasonix port.** Same TDD discipline as the Claude Code original, with two adaptations: (1) Writ's enforcement hooks do **not** fire on Reasonix, so the test-first / write-block gates are **advisory only** — this skill's own discipline (one phase, test-before-code, present-before-implement) is the guarantee, not a mechanical gate. (2) The guardrail-audit subagent (Step 4b) is dispatched via Reasonix's `task` tool / `run_skill`, not a named agent. The `writ-mode-set` bash is a harmless no-op on Reasonix.
...
**Approval (Reasonix):** there is no mechanical write-block here — Writ's pre-write gate
is a Claude Code hook that does not fire on Reasonix. So approval is by **explicit
go-ahead**: present the phase plan above, then **wait for the user's go-ahead** before
writing any code or test. This skill's discipline (present-before-implement, one phase at
a time) is the only gate. Do not write code until the user approves the phase scope.
...
Dispatch a subagent via the Reasonix `task` tool (generic subagent) with a `prompt` that contains:
...
When verified, run: /reasonix-tdd-implement $FEATURE for Phase <N+1>.
```
