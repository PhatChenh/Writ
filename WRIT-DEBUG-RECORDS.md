...
### B20 — Plugin installs ship NO writ allowlist → every writ Bash command hard-blocks when the auto safety-classifier is offline — ✅ FIXED 2026-06-29
...
- **Fix (2026-06-24):**
  ...
  - **(2) Bootstrap seeding — ✅ verified 2026-06-29.** New step "6c" in `scripts/bootstrap-plugin.sh` seeds the same 13 rules into `~/.claude/settings.json` via an additive + idempotent + order-preserving jq filter, so future installs are covered.
- **Verified (2026-06-29):** `bash -n scripts/bootstrap-plugin.sh` → SYNTAX OK. jq filter dry-run on temp files, 3 cases all pass: empty `{}` → 13 rules; additive to existing rules → order-preserving + dedup (an already-present `*writ-session.py *` is NOT re-added); idempotent re-run → 13 unique, no dupes. Cache synced (`cp scripts/bootstrap-plugin.sh → ~/.claude/plugins/cache/writ/writ/1.5.0/scripts/`, 4 markers confirmed). **Full E2E against the live `~/.claude/settings.json`** (ran via `/tmp/b20_seed.sh` through the user's `!` prefix to bypass the classifier outage — the exact condition B20 fixes): backup created, seeding applied, 13 writ rules present (missing `[]`), all 11 top-level keys preserved (`agentPushNotifEnabled, effortLevel, enabledPlugins, env, extraKnownMarketplaces, hooks, model, permissions, statusLine, theme, tui` — nothing clobbered), valid JSON + no dupes, idempotent re-run produces an identical file. Live settings went 9 → 13 writ rules (added the 4 missing: `check-gates.sh`, `verify-files.sh`, `scan-deps.sh`, `validate-handoff.sh`). No daemon touched. **Requires Claude restart** for the new permission rules to load in any in-flight session.
...
## Session 2026-06-29 — Iris Phase 6 orchestration (Claude driving deepseek; code in git worktrees). Reporter observations, ✅ FIXED 2026-06-29.
...
**Fix (2026-06-29, `bin/run-analysis.sh:552-606`, option "Both"):** the missing-auth-decorator scan now recognizes THREE manual-auth signals and widened its scan window from 6 lines to the **full handler body** (stacked decorators + def header + indented body, via new `_handler_body(lines, i)` — a manual auth call deeper in the body, as in `api/query.py`, is now seen):
  - **(1) validator call** — `MANUAL_AUTH` matches a call to a known validator inside the body: `validate_api_key|verify_api_key|verify_token|verify_credentials|authenticate_request|authenticate|check_auth|require_auth|require_admin|require_role|get_current_user|current_user|resolve_principal|authorize_request|is_authorized` followed by `(`. A call (not a mention) — `current_user(` not `current_user =`.
  - **(2) credential header + 401** — `CRED_HEADER` (`X-API-Key|X-Auth-Token|Authorization|api_key|access_token|bearer`) AND `UNAUTHORIZED` (`401|HTTPException(..401..)|abort(401)|status_code=401|raise Unauthorized|return ..401`) both present in the body.
  - **(3) per-route `# writ:manual-auth` escape directive** — on the decorator line or first body line; operator asserts a reviewed manual-auth route the heuristics still miss. (Distinct from B29's file-level `# writ:noauth`, which stays for intentionally auth-less modules.)
  - A route is flagged only if NONE of decorator-auth (existing `AUTH_GUARD`) / manual-auth / the directive apply. Hard-block severity unchanged — still blocks genuinely unauthed routes; the FP on manual-auth routes is gone.
  - **Verified:** 8-case suite against `bin/run-analysis.sh` — `manual_auth.py` (validate_api_key+401) → 0 (was the B26 FP, now cleared); `manual_deep.py` (validator call 15 lines into body) → 0 (old 6-line window would have missed + blocked); `cred_401.py` (header+401, no validator) → 0; `manual_directive.py` (`# writ:manual-auth`) → 0; `decorator_auth.py` (`@login_required`) → 0; `depends_auth.py` (`Depends(get_current_user)`) → 0; `noauth_file.py` (file-level `# writ:noauth`) → 0; `unauthed.py` (bare route) → 1 (still hard-blocked, correct rule/shape). `bash -n` clean. **Synced to `~/.claude/plugins/cache/writ/writ/1.5.0/bin/run-analysis.sh`** (7 markers confirmed); the scanner is a shell script, bash re-reads it each invocation → **no Claude restart needed** (per B22 caveat).
...
**Fix (2026-06-29, option "HTTP author endpoint" — daemon authors in-process, owns the lock, no conflict):**
  - **`writ/server.py`:** new `POST /author-project-rule` endpoint + `AuthorProjectRuleRequest` Pydantic model (the 9 REQUIRED_FIELDS + `source_attribution` + `force` + `rules_dir`). `bin/lib/project_rules.py` is loaded via `importlib` (mirrors the writ_session load) so the daemon can call `_project_rules._author(_db, candidate, Path(rules_dir), force)` IN-PROCESS — the daemon's `_db` already owns the graph lock, so there is no second-connection deadlock and NO daemon bounce. `export|list` still use the direct `with_daemon_paused` path (out of B27 scope; they bounce once/phase, documented).
  - **`bin/lib/project_rules.py`:** new `author-http` subcommand (shares args with `author` via `_add_author_args`), `_resolve_writ_port()` (mirrors `writ-session.py:_resolve_writ_port` + `common.sh` — `WRIT_PORT_OVERRIDE` wins, else `8765 + cksum(WRIT_REPO_ROOT.rstrip("/")) % 1000`, cksum shelled out for byte-exact hash match), and `_author_http(candidate, rules_dir, force)` POSTs via `urllib` to `http://127.0.0.1:{port}/author-project-rule`. `main()` handles `author-http` BEFORE `_connect()` (so it opens NO direct graph connection — the whole point).
  - **`bin/writ-project-rules.sh`:** the `author` case now routes — `daemon_up` → `run_engine author-http` (no `with_daemon_paused`, so NO daemon kill/restart — safe with concurrent sessions on the same daemon); else → `run_engine author` (direct, no lock holder). `export|list|seed` unchanged.
  - **Verified:** `bash -n` + `py_compile` clean; `_resolve_writ_port()` == `common.sh` port for this repo (9041==9041, port-agreement YES); `/author-project-rule` registered in `app.routes`; mock-server E2E (GET /health → 200, POST /author-project-rule → echoes body + `accepted:true`) with `WRIT_PORT_OVERRIDE` → shell routed `author` through HTTP, `_author_http` POSTed all 9 fields + `source_attribution` + `force` + `rules_dir` correctly, response `accepted:true`, exit 0.
  - **Full E2E against an ephemeral daemon running the NEW `server.py`** (temp port 19527 + temp CWD/graph — the live 9041 daemon was NOT touched, two other agents stayed on it): `WRIT_PORT_OVERRIDE=19527 WRIT_REPO_ROOT=<tmp> WRIT_ALLOW_EMBEDDING_FALLBACK=1` + `writ_spawn_daemon_detached` → daemon healthy (rule_count 0). Then `writ-project-rules.sh author --rule-id PROJ-E2E-001 …` → shell routed through HTTP (daemon_up) → endpoint called `_project_rules._author(_db,…)` in-process → `accepted:true, authority:human, export.files_written:1`. Rule **persisted to graph.db** (`/health rule_count` 0→1, survived a daemon stop+restart re-warm) and **exported to `docs/rules/e2etest/rules.md`** (full content). Authoritative `project_rules.py list --json` from the temp CWD → `PROJ-E2E-001 | authority=human | domain=E2ETest | scope=backend`. 9041 healthy throughout. (Pre-existing `GET /rule/{rule_id}` returns "not found" for a `get_rule` vs `get_all_rules` discrepancy in the DB layer — unrelated to B27; `get_all_rules`/export/`list` all see the rule.)
  - **⚠️ Live-daemon caveat (NOT E2E-verified against the live 9041 daemon itself):** `server.py` + `project_rules.py` are Python modules loaded at daemon startup — the in-process endpoint takes effect on the live 9041 daemon ONLY when it restarts. Two other agents were on 9041, so it was NOT restarted. Until the restart, `author` against the live (old) daemon → new shell routing → `author-http` → POST → 404 (endpoint absent) → exit 2 (graceful, no daemon disruption — the new shell never calls `with_daemon_paused` for `author`). The E2E above proves the code path is correct; only the live-daemon restart remains to activate it there.
  - **Synced to `~/.claude/plugins/cache/writ/writ/1.5.0/`** (`writ/server.py` 8 markers, `bin/lib/project_rules.py` 8 markers, `bin/writ-project-rules.sh` 1 marker, confirmed). The shell + python files are in the cache; `writ-project-rules.sh` takes effect immediately (bash re-reads each invocation). **Only the live 9041 daemon restart remains owed** to activate the in-process endpoint there. (A transient classifier outage — the B20 condition itself — briefly blocked the `cp`; it cleared and the sync completed.)
