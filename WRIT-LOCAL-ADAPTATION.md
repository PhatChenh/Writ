...
|---|---|---|---|---|
| 1 | **writ-rag-inject → A-auto** (D4-02) | per-repo daemon: prompt→RAG inject each turn + daemon auto-start | 3 edits to the hook: `cd` **repo root** not `WRIT_DIR`; per-repo `WRIT_PORT = 8765 + hash(git_root)%1000`; per-port start-lock | **FOUNDATION** — every server hook (RAG, `/pre-write-check`, `/analyze`, `/query`) needs the daemon up & per-repo. No deps. Do first. |
