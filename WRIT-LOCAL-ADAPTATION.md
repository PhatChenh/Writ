...
**#4 OPEN ITEM (needs live plugin to resolve):** design/spec gates are **Work-mode-only**, but build-pipeline arms Work mode only at the *plan* step → design/spec artifacts (written earlier in conversation mode, by subagents) may not trigger their gates. Ties to the deferred **"Work-at-design"** decision (now unblocked since #3 adapted validate-design-doc). Resolution = arm Work mode at the design step AND/OR confirm subagent mode propagation — both need a live daemon to verify.
