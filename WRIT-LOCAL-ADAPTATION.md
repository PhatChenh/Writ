...
| 2 | **Mode auto-switch wiring** (D4-03) | our skills set Writ mode on entry (deterministic, not heuristic) | add `mode set <x>` to skill entry points: `/grill`→conversation, `/build-pipeline` implement→work, `/code-review`→review, `/tdd-implement`→work | **FOUNDATION** — every Work-gated hook + the gate machine is inert without it. Touches OUR skills, no Writ-core. Dep: 1. |
...
**Where we are:** Phase 4 adaptation **implementation** in progress on branch `phase4-adaptation` (off `main`). Hook decisions all locked (ledger above: 31 keep / 2 drop). Working the ordered **Adaptation work plan** (table above).
...
**Next:** Work-plan **#2 — mode auto-switch wiring** (D4-03): add `mode set <x>` to our skill entry points (`/grill`→conversation, `/build-pipeline` implement→work, `/code-review`→review, `/tdd-implement`→work). Then #3 path/template batch, #4 gate reconciliation, #5 review flow, #6 4A/4B/4C, #7 enhancements.
